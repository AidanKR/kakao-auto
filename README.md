...
## 2D 관계망 시각화
`graph_export.py` / `9_viz.bat` — DB에서 **방↔사람 소통 관계망**을 뽑아 **2D 인터랙티브 HTML** 생성(**노드 클릭=우측 패널에 그 방/사람의 실제 대화** · 드래그 이동 · 휠 확대 · 팬 · 이름 검색). **완전 자체완결**(외부 라이브러리·CDN 없음) → 인터넷 없이 더블클릭이면 열림. LLM·외부전송 0(전부 로컬). 활동 많은 방/사람이 크게, 소통 허브가 한눈에.
