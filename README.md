...
**무인 24시간 자동(exe):** KakaoAuto 메뉴에서 **`11` (무인 자동 설정)** 을 한 번 누르면 — ①PC 로그인 시 수집 자동시작(멈추면 자동 재시작) ②정리 하루 2번(11:50·23:50) 자동 예약이 등록됩니다(관리자 권한 불필요). 카톡은 "PC 켤 때 자동 실행 + 자동 로그인"을 켜두세요. 해제는 **`12`**. (명령줄로도: `KakaoAuto.exe collect` / `consolidate` / `autostart` / `autostart-off`)
...
## 금액·약속·계좌 자동 추출
`extract.py` / `13_extract.bat` — 대화에서 **금액**(50,000원·5만원·3억)·**약속**(내일 3시·8월 5일)·**계좌**(은행+번호)를 정규식으로 뽑아 CSV(엑셀)로. LLM 없이 오프라인. 카톡 주문/정산을 손으로 옮겨 적는 시간을 줄여줍니다.

## 사진·파일 만료 전 백업
`media_backup.py` / `14_media.bat` — 카톡이 **이미 다운로드/캐시한** 사진·파일을 아카이브(기본 `share_dir/media`)로 복사(sha1 중복제거). 카톡 서버에서 지워지기 전에 로컬 미디어를 보존합니다.
- **한계(정직):** ①'아직 안 본(다운로드 안 된)' 미디어는 못 가져옵니다(열어야 받아짐) → 자주 돌릴수록 많이 건짐. ②카톡 캐시 폴더는 버전마다 다름 → 첫 실행 로그에 뜬 폴더를 보고 `config` 의 `media_dirs` 를 맞추세요. 자주 백업하려면 작업 스케줄러에 등록.

## 단일 실행파일(exe) — 비개발자용
`kakao.py`(통합 메뉴 런처) + `build_exe.bat`(PyInstaller). 윈도우에서 `build_exe.bat` 한 번 실행하면 `dist\KakaoAuto.exe` 가 생깁니다. 그 exe 하나로 **파이썬 설치 없이** 수집·정리·대시보드·검색·관계망·엑셀·추출·백업을 메뉴로 실행. `config.json`·`kakao.db`·출력물은 **exe 옆 폴더**에 저장됩니다(`appdir` 로 frozen 대응).
